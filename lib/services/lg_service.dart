...
// I ADDED THIS IMPORT TO LOAD THE IMAGE ASSET
...
class LgService extends ChangeNotifier {
  ...
  Future<void> initializeConnection() async {
    ...
    try {
      // 1. LOAD SETTINGS FROM STORAGE (We ask the model to read the shared preferences)
      LgConnectionModel savedModel = await LgConnectionModel.loadFromPreferences();

      // 2. UPDATE THE "CREDENTIALS" WITH THOSE VALUES
  ...
  // NEW sendLogo() METHOD FOR SENDING LG'S LOGO
  Future<bool> sendLogo() async {
    if (_client == null) return false;

    try {
      // UPLOAD IMAGE FROM assets/images
      final ByteData data = await rootBundle.load('assets/images/LiquidGalaxyLogo.png');
      final sftp = await _client!.sftp();
      final remoteFile = await sftp!.open('/var/www/html/LiquidGalaxyLogo.png',
          mode: SftpFileOpenMode.create | SftpFileOpenMode.truncate | SftpFileOpenMode.write);
      await remoteFile.write(Stream.value(data.buffer.asUint8List()));
      await remoteFile.close();

      // IDENTIFY THE LEFTMOST SCREEN BASED ON THE NUMBER OF SCREENS IN THE LG RIG
      int leftMostScreen = calculateLeftMostScreen(_lgConnectionModel.screens);

      // KML SCRIPT THAT POINTS TO OUR LiquidGalaxyLogo.png LOCATED IN assets/images
      String kmlContent = '''
...
<Document>
    ...
    <ScreenOverlay>
        <name>Logo</name>
        <Icon>
            <href>http://lg1:81/LiquidGalaxyLogo.png</href>
        </Icon>
      ...
      // SEND AND EXECUTE IMAGE
      final result = await execute(
        "echo '$kmlContent' > /var/www/html/kml/slave_$leftMostScreen.kml",
        'Logo successfully sent',
      );

      return result != null;
    } catch (e) { // IF LOGO ISN'T SENT PROPERLY, PRINT AN ERROR MESSAGE IN THE CONSOLE
      debugPrint("Error sending logo: $e");
      return false;
    }
  }

  // FLY TO XÀTIVA (MY HOMETOWN)
  Future<void> flyToXativa() async {
    // THE EXACT COORDINATES WITHIN THE CITY POINT TO THE EXACT SAME PLACE WHERE THE PYRAMID SHOULD APPEAR
    String flyToCmd = 'flytoview=<LookAt><longitude>-0.5216</longitude><latitude>38.9887</latitude><altitude>0</altitude><heading>0</heading><tilt>60</tilt><range>800</range><gx:altitudeMode>relativeToGround</gx:altitudeMode></LookAt>';

    await execute('echo "$flyToCmd" > /tmp/query.txt', 'Flying to Pyramid');
  }

  // SEND PYRAMID FUNCTION
  Future<void> sendPyramid() async {
    try {
      // READ THE PYRAMID.KML FILE
      String kmlContent = await rootBundle.loadString('assets/kml/pyramid.kml');

      // WE REMOVE THE LINE BREAKS SO ECHO DOESN'T BREAK (ABSOLUTELY NEEDED TO WORK)
      String safeKml = kmlContent.replaceAll('\n', ' ');

      // UPLOAD THE MODIFIED .KML FILE WITHOUT LINE BREAKS
      await execute("echo '$safeKml' > /var/www/html/pyramid.kml", 'Pyramid uploaded');

      // DISPLAY THE PYRAMID
      await execute('echo "http://lg1:81/pyramid.kml" > /var/www/html/kmls.txt', 'Pyramid displayed');
    } catch (e) {
      debugPrint("Error sending pyramid: $e");
    }
  }
